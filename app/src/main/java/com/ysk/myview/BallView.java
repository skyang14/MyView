...
/**悬浮碰撞小球，带有点击数据
 * Created by yang.shikun on 2020/4/3 11:13
 */
...
public class BallView extends View {
    ...
    private float stokeWidth = dp2px(getContext(),5);
    private int stoke = dp2px(getContext(),5);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        maxRadius = mWidth / 100;
        minRadius = mWidth / 100;
        // 初始化圆的半径和圆心
        for (int i = 0; i < mCount; i++) {
            if (datas.size() > 0) {
                if ("1".equals(datas.get(i))) {
                    mBalls[i].radius = mWidth / 14+stoke;
                } else if ("2".equals(datas.get(i))) {
                    mBalls[i].radius = mWidth / 13+stoke;
                } else if ("3".equals(datas.get(i))) {
                    mBalls[i].radius = mWidth / 12+stoke;
                } else if ("4".equals(datas.get(i))) {
                    mBalls[i].radius = mWidth / 11+stoke;
                } else if ("5".equals(datas.get(i))) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (datas.size() > 0) {
            for (int i = 0; i < datas.size(); i++) {
                ...
                ball.content = datas.get(i);
                ball.taskId = datas.get(i);
...
//            canvas.drawText(str, rect.centerX(), baseLineY, tPaint);
    ...
    //由于是动态刷新，所以在加载数据的时候需要调用requestLayout
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            for (int i = 0; i < mCount; i++) {
                ...
                if (distanceZ <= ball.radius && progressListener != null) {
